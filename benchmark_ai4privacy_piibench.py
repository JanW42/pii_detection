...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark AI4Privacy PII model on Pritesh-2711/pii-bench or span-based PII datasets."
    )
    parser.add_argument("--model", default="ai4privacy/llama-ai4privacy-multilingual-categorical-anonymiser-openpii")
    parser.add_argument("--dataset", default="Pritesh-2711/pii-bench")
    ...
    language = (args.language or args.filter_language or "en").strip().lower()
    args.language = language
    args.filter_language = language
    if args.plot_path is None:
        args.plot_path = f"benchmark_ai4privacy_confusion_matrix_{language}.png"
    if args.output_path is None:
        args.output_path = f"benchmark_ai4privacy_results_{language}.txt"
    return args
...
def get_text(row: dict) -> str:
    for key in ("source_text", "text", "raw_text", "source"):
        value = row.get(key)
...
def stringify_label(label, label_names: list[str] | None = None) -> str:
    if isinstance(label, int) and label_names and 0 <= label < len(label_names):
        return label_names[label]
    return str(label)


def get_label_names(dataset) -> list[str] | None:
    try:
...
def normalize_pii_label(label: str) -> str:
    normalized = str(label).strip().upper()
...
def align_tokens_to_text(text: str, tokens: list[str]) -> list[tuple[int, int] | None]:
...
def build_spans_from_token_labels(
    offsets: list[tuple[int, int] | None],
    labels: list[str],
) -> list[tuple[int, int]]:
    spans: list[tuple[int, int]] = []
    ...
    for offset, label in zip(offsets, labels):
        is_pii = pii_label_is_positive(label)
        normalized = normalize_pii_label(label)
        starts_new = str(label).strip().upper().startswith("B-")

        if offset is None or not is_pii:
        ...
        start, end = offset
        if current_start is None or starts_new or normalized != previous_entity:
            if current_start is not None and current_end is not None:
                spans.append((current_start, current_end))
            current_start = start
            current_end = end
...
def get_token_label_truth(
    row: dict,
    text: str,
    label_names: list[str] | None,
) -> tuple[list[tuple[int, int] | None], list[bool], list[tuple[int, int]]] | None:
    tokens = row.get("tokens")
    labels = row.get("labels")
    if not isinstance(tokens, list) or not isinstance(labels, list) or len(tokens) != len(labels):
        return None

    label_texts = [stringify_label(label, label_names) for label in labels]
    offsets = align_tokens_to_text(text, [str(token) for token in tokens])
    y_true = [pii_label_is_positive(label) for label in label_texts]
    gt_spans = build_spans_from_token_labels(offsets, label_texts)
    return offsets, y_true, gt_spans
...
def main() -> None:
    ...
    elif args.filter_language:
        print(
            f"Dataset has no 'language' column; cannot filter to '{args.filter_language}'. "
            f"Running all rows from split '{args.split}'."
        )
    ...
    token_truth_batch: list[tuple[list[tuple[int, int] | None], list[bool], list[tuple[int, int]]] | None] = []
    ...
    def flush_batch() -> None:
        ...
        for row, text, gt_spans, token_truth, pred_spans in zip(
            rows_batch,
            texts_batch,
            gt_spans_batch,
            token_truth_batch,
            ...
            elif token_truth is not None:
                truth_offsets, truth_labels, _ = token_truth
                y_true = truth_labels
                y_pred = mark_offsets_as_pii(truth_offsets, pred_spans)
            else:
    ...
    for row in tqdm(dataset, total=len(dataset), desc="Benchmark", unit="sample", dynamic_ncols=True):
        ...
        token_truth = get_token_label_truth(row, text, label_names)
        gt_spans = token_truth[2] if token_truth is not None else get_ground_truth_spans(row)
        rows_batch.append(row)
        texts_batch.append(text)
        gt_spans_batch.append(gt_spans)
        token_truth_batch.append(token_truth)
