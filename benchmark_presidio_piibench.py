...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark Presidio on Pritesh-2711/pii-bench or span-based PII datasets."
    )
...
def main() -> None:
    ...
    for row in tqdm(dataset, total=len(dataset), desc="Benchmark", unit="sample", dynamic_ncols=True):
        ...
        token_label_truth = get_token_label_truth(row, text, label_names)
        if token_label_truth is None:
            gt_spans = get_ground_truth_spans(row)
        else:
            truth_offsets, truth_labels, gt_spans = token_label_truth
        ...
        elif token_label_truth is not None:
            y_true = truth_labels
