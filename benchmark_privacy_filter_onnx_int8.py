...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark openai/privacy-filter on ai4privacy/open-pii-masking-500k-ai4privacy validation split."
    )
    ...
    parser.add_argument(
        "--runtime",
        choices=["torch", "onnx-int8"],
        default="torch",
        help="Inference runtime: PyTorch oder ONNX INT8 (CPU).",
    )
    parser.add_argument(
        "--plot-path",
...
def build_onnx_int8_session(model_name: str) -> ort.InferenceSession:
    local_dir = snapshot_download(
        repo_id=model_name,
        allow_patterns=["onnx/model_quantized.onnx", "onnx/model_quantized.onnx_data*"],
    )
    model_path = f"{local_dir}/onnx/model_quantized.onnx"
    return ort.InferenceSession(model_path, providers=["CPUExecutionProvider"])


def predict_spans_for_batch_onnx_int8(
    texts: list[str],
    tokenizer: AutoTokenizer,
    ...
    if "token_type_ids" in encoded:
        ort_inputs["token_type_ids"] = encoded["token_type_ids"].cpu().numpy().astype(np.int64, copy=False)
...
def main() -> None:
    ...
    if args.runtime == "onnx-int8":
        if device.type != "cpu":
            raise ValueError("ONNX INT8 wird in diesem Skript nur auf CPU unterstuetzt. Nutze --device cpu.")
        session = build_onnx_int8_session(args.model)
    ...
    def flush_batch() -> None:
        ...
        if args.runtime == "onnx-int8":
            pred_spans_batch = predict_spans_for_batch_onnx_int8(texts_batch, tokenizer, session, id2label)
        else:
